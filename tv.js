...
'use strict';
    ...
    function applyProxy(url) {
        ...
        // ⛔️ НЕ проксировать повторно
        if (url.indexOf('/proxy?url=') !== -1) return url;
        if (!Lampa.Storage.get('hack_tv_proxy_enabled', false)) return url;
        var host = (Lampa.Storage.get('hack_tv_proxy_host', '') || '').trim();
        if (!host) return url;
        if (!/^https?:\/\//i.test(host)) host = 'http://' + host;
        host = host.replace(/\/$/, '');
        return host + '/proxy?url=' + encodeURIComponent(url);
    }
    ...
    */
    ...
    function pluginPage(object) {
        ...
        this.activity.loader(true);
                // ⚠️ предупреждение про localhost на TV / Android
                (function(){
                    ...
                    if (host.indexOf('127.0.0.1') === -1 && host.indexOf('localhost') === -1)
                        return;
                    // если не ПК (эвристика)
                    if (!Lampa.Platform || Lampa.Platform.tv || Lampa.Platform.android) {
                        Lampa.Noty.show('Прокси на localhost недоступен для TV/Android. Используй IP ПК в сети.',
                            5000
                        );
                    }
                })();
        ...
        var emptyResult = function () {
            Lampa.Noty.show(
                'Не удалось загрузить плейлист. Проверь прокси или URL.',
                3000
            );
        ...
        if (Object.keys(catalog).length) {
            ...
            network.native(
                ...
                function () {
                    network.silent(
                        Lampa.Utils.protocol() + 'epg.rootu.top/cors.php?url=' + encodeURIComponent(rawUrl)
                        + '&uid=' + utils.uid()
                        + '&sig=' + generateSigForString(rawUrl),
                        compileList,
                        emptyResult,
                        false,
                        { dataType: 'text' }
