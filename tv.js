...
// --- ТВОЯ ЛОГИКА УМНОГО ПРОКСИРОВАНИЯ ---
...
function bulkWrapper(func, bulk) {
    ...
    return function() { queue.push(arguments); runner(); }
}
...
function pluginPage(object) {
    ...
    this.create = function () {
        ...
        // Если данные уже загружены для этого URL, берем из кеша
        if (catalog[object.url]) {
            _this.build(catalog[object.url]);
        } else {
            network.native(prepareUrl(object.url), function (data) {
                var parsed = _this.parse(data);
                catalog[object.url] = parsed;
                _this.build(parsed);
            }, function () {
    ...
    this.build = function (groups) {
        ...
        var currentGroupName = object.group || 'Все';
        var channels = groups[currentGroupName] || [];
    ...
    // ГЛАВНОЕ: Кнопка меню слева (Категории)
    this.menu = function () {
        var menu = [];
        var groups = catalog[object.url];
        for (var g in groups) {
            menu.push({
                title: g,
                group: g
    ...
    // ГЛАВНОЕ: Переключение категорий
