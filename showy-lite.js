...
var BASE = 'http://89.110.72.185:9118';
window.showy_debug = true;
...
var hostkey = BASE.replace('http://', '').replace('https://', '');
...
window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
  if (!window.rch_nws[hostkey].startTypeInvoke) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent(BASE.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke(BASE, function() {
 ...
 if (url === 'eval' || url === 'evalrun') {
  console.warn('RCH eval disabled');
  result('');
}
 else if (url == 'ping') {
        result('pong');
      } else {
        if (window.showy_debug) console.log('RCH', url);
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript([BASE + "/js/nws-client-es5.js?v18112025"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(account(BASE + '/lite/withsearch'), function(json) {
  ...
  if (false) {
    if (Platform.is('browser') && location.host.indexOf("127.0.0.1") !== -1) {
      ...
      $.get(BASE + '/player-inner/' + element.url);
      return;
    }
          ...
          try {
            ...
            Lampa.Activity.push({
              ...
              page: 1
                        ...
                        console.error('Showy Lite launch error', e);
                    }
