;(function () {
    ...
    // Состояние прокси (берем из памяти или ставим по умолчанию)
    var proxy_enabled = Lampa.Storage.get('hack_tv_proxy_enabled', 'false');
    var proxy_address = Lampa.Storage.get('hack_tv_proxy_address', 'http://192.168.1.50:7777');

    // --- ЛОГИКА СТРАНИЦЫ ПЛАГИНА ---
    function pluginPage(object) {
        ...
        this.create = function () {
            var _this = this;
            // Здесь будет твой основной интерфейс списка каналов
            var info = $('<div class="selector" style="padding: 1.5em; background: rgba(255,255,255,0.05); margin: 10px; border-radius: 0.5em;">' +
                '<h2>' + plugin.name + '</h2>' +
                '<p>Прокси: ' + (Lampa.Storage.get('hack_tv_proxy_enabled', 'false') === 'true' ? 'ВКЛ' : 'ВЫКЛ') + '</p>' +
                '<p>Адрес: ' + Lampa.Storage.get('hack_tv_proxy_address', '---') + '</p>' +
                '</div>');
    ...
    function addSettings() {
        ...
        var item = $('<div class="settings-folder selector" data-name="hack_tv_settings" data-children="true">' +
            ...
            '<div class="settings-folder__descr">Настройки проксирования потока</div>' +
        '</div>');
        ...
        item.on('hover:enter', function () {
            Lampa.Select.show({
                ...
                items: [
                    {
                        ...
                        type: 'select',
                        values: {'false': 'Выключено', 'true': 'Включено'},
                        default: 'false'
                    },
                    {
                        title: 'Адрес прокси ПК',
                ...
                onSelect: function (a) {
                    ...
                    Lampa.Noty.show('Сохранено: ' + a.value);
                },
                onBack: function () {
        ...
        // Важно обновить контроллер, чтобы новая кнопка стала доступна для выбора
        if (Lampa.Controller.active().name === 'settings_main') Lampa.Controller.update();
    }

    // --- КНОПКИ ПУЛЬТА ---
    function handleKeyDown(e) {
        // Код выполняется только если мы ВНУТРИ нашего плагина
        if (Lampa.Activity.active().component === plugin.component) {
            if (e.code >= 48 && e.code <= 57) {
                Lampa.Noty.show('Нажата цифра: ' + (e.code - 48));
                e.preventDefault();
            }
        }
    }
    ...
    function startPlugin() {
        if (window.hack_tv_skeleton_ready) return;
        window.hack_tv_skeleton_ready = true;

        // Регистрируем страницу
        Lampa.Component.add(plugin.component, pluginPage);

        // Добавляем в левое меню (с защитой от дублей)
        if (!$('.menu__item[data-action="' + plugin.component + '"]').length) {
        ...
        Lampa.Settings.listener.follow('open', function (e) {
            if (e.name === 'main') {
                // Небольшая задержка, чтобы DOM настроек успел построиться
                setTimeout(addSettings, 100);
            }
        });

        // Слушаем кнопки пульта
